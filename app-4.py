...
# FINAL WORKING VERSION – RESTORED EXACTLY TO ORIGINAL
# + FIXED reference split bug
# + FIXED length/reference rules only
#############################################################
...
# SIDEBAR
# ==========================================================
st.sidebar.header("Upload Files (Two Required)")
...
if not (PARQUET and EMB_PATH):
    st.warning("Please upload both required files.")
    st.stop()
...
# ==========================================================
# GPT REVIEW FUNCTION (UNCHANGED STRUCTURE)
# ==========================================================
...
Rewrite the research gap using AT LEAST 200 words.
Title: {title}
Gap: {gap}
References: {refs}
...
# MAIN UI
# ==========================================================
st.title("AI-BIM / Research Gap Evaluation App")

title = st.text_input("Enter Dissertation Title")
gap = st.text_area("Paste Research Gap", height=180)
refs = st.text_area("Paste APA References (1 per line)", height=150)
...
if st.button("Evaluate Research Gap"):
    with st.spinner("Evaluating..."):
        ...
        # ======================================================
        # FIXED: REFERENCE SPLIT — THIS WAS YOUR BROKEN LINE
        # ======================================================
        ...
        col3.markdown(f"<div class='metric-card'><div class='metric-title'>Significance</div><div class='metric-value'>{gpt['significance_score']}/10</div></div>", unsafe_allow_html=True)
        col4.markdown(f"<div class='metric-card'><div class='metric-title'>Verdict</div><div class='metric-value'>{verdict}</div></div>", unsafe_allow_html=True)

        tab1, tab2, tab3, tab4 = st.tabs(["Top Literature", "Reviewer Comments", "Weaknesses", "Rewritten Gap"])
        ...
        with tab2:
            ...
            st.write("**Novelty Rationale:**", gpt["novelty_rationale"])
            st.write("**Significance Rationale:**", gpt["significance_rationale"])
            st.write("**Clarity Rationale:**", gpt["clarity_rationale"])
            st.write("**Citation Rationale:**", gpt["citation_rationale"])

        with tab3:
            st.write("### Improvements")
            for item in gpt["improvements"]:
                st.write("- ", item)

            st.write("### Good Points")
            for item in gpt["good_points"]:
                st.write("- ", item)
