...
# ------------------------------------------------------------
# File paths (must match the files you uploaded to Streamlit Cloud)
# ------------------------------------------------------------
DATA_PATH_1 = "dt_construction_filtered_topics (1).csv"
...
# ------------------------------------------------------------
missing = []
for f in [DATA_PATH_1, DATA_PATH_2, EMB_PATH]:
    if not os.path.exists(f):
        missing.append(f)

if missing:
    st.error(
        "❌ The following required data files are missing from Streamlit Cloud:\n"
        + "\n".join(missing)
        + "\n\nUpload them via: Manage App → Files → Upload."
    )
    st.stop()
...
if st.button("Check Relevance (Top 25 Papers)"):
    ...
    if emb is None:
        ...
        st.download_button("Download CSV", csv, "top25_relevant_studies.csv")
